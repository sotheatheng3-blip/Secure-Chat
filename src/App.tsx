...
import { MessageSquare, Server, Mail, Lock as LockIcon, User as UserIcon, Smile, Settings, Plus, BookOpen, Key, LogOut, RefreshCw, Phone, PhoneOff, Mic, MicOff, Video, VideoOff, Volume2 } from "lucide-react";
...
export default function App() {
      ...
      try {
        ...
        switch (type) {
          ...
          case "sync:active_keys": {
            ...
            // Update activeUsers array with usernames, status, and avatars
            setActiveUsers((prev) => {
          ...
          case "user:presence": {
            const { username: presUsername, publicKey: presKey, avatar: presAvatar, status } = payload;
            if (status === "online") {
              setActiveUsers((prev) => {
                const filtered = prev.filter(u => u.username !== presUsername);
                return [...filtered, { username: presUsername, status: "online", publicKey: presKey || "none", avatar: presAvatar || "🦊" }];
              });
              logSecurity(`Presence: User @${presUsername} is online`);
            } else {
              setActiveUsers((prev) => {
          ...
          case "message:received": {
            ...
            if (message.sender !== userHandle) {
              ...
              if (isRoomNotActive || isDocHidden) {
                if (typeof window !== "undefined" && "Notification" in window && Notification.permission === "granted") {
                  ...
                  if (targetRoom) {
                    if (targetRoom.isGroup) {
                      notifTitle = `#${targetRoom.name} • @${message.sender}`;
                    } else {
  ...
  const handleLoginIdentity = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error || "Failed to login. Please check qualifications.");
      }

      const { username: usr, avatar: av } = data.user;
  ...
  const handleUpdateProfile = async (
    ...
    try {
      ...
      logSecurity(`Profile updated on database. Synchronizing username @${newUsername} and avatar ${newAvatar}...`);
